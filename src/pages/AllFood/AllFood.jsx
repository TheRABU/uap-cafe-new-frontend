...
const AllFood = () => {
  ...
  const fetchFoods = async (query) => {
    try {
      setLoading(true);
      setError(null);
      const res = await axios.get(`/foods?search=${query}`);
      setFoods(res.data);
    } catch (err) {
  ...
  const debouncedFetchOrders = useCallback(
    debounce((query) => fetchFoods(query), 300),
    []
  );
              ...
              <tbody>
                {foods.map((food, index) => (
                  ...
                  >
                    ...
                    <td className="p-3 py-4">{food?.name || "N/A"}</td>
                    <td className="p-3 py-4">{food?.sellerEmail || "N/A"}</td>
                    <td className="p-3 py-4">{food?.category || "N/A"}</td>
                    <td className="p-3 py-4">{food?.price || "N/A"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
